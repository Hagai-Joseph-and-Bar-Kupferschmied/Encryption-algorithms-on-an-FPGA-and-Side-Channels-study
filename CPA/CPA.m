path_trace_folder = './Traces/'; % path for the folder containing all the traces
path_plain_text = './plainText.txt'; % path for the plain text used
path_key = './CypherKey.txt'; % path of the file in which the key will be written
path_Sbox = './SBox.mat'; % path of the S-Box (.mat format)
nb_trace = 1000; % number of traces saved
nb_points = 5000; % number of points for each trace - needed to be confirm
nb_bytes = 16; % lenght of the cypher key in bytes (16*8=128)
...
matrix_subbytes = load(path_Sbox, '-mat');
...
GetResults(key, vec_corr, path_key);
plotCorrelation(nb_points, matrix_trace, vec_time);

function plotCorrelation(p_nb_points, p_matrix_traces, p_vec_time)
...
function GetResults(key_string, vec_corr, file_path)
